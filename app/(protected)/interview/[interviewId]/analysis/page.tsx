...
const Analysis: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAnalysis = async () => {
      try {
        ...
        // Convert structured data to array format
        const transformedData = Object.keys(structuredData[0])
        .filter((key) => ["Introduction", "Project", "Coding", "Technical", "Outro"].includes(key))
  ...
  useEffect(() => {
    if (analysisData && analysisData.length > 0) {
      console.log("Received Analysis Data in analysis page : ", analysisData);
  ...
  return (
    <div className="h-screen w-full relative">
      ...
      <ScrollArea className="px-8 py-4 h-[calc(100vh-2rem)] ">
        {analysisData?.length===0&& <div className="flex justify-center items-center h-full">
