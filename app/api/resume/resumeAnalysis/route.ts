...
import { updateResumeAnalysis } from "@/actions/resumeAnalysis";
...
export const POST = async (req: NextRequest) => {
  ...
  if (!baseUrl) {
    return NextResponse.json(
      {
        message:
          "NEXT_PUBLIC_RESUME_API_ENDPOINT environment variable is not set",
      },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `Failed to fetch summary: ${response.status} - ${errorText}`
      );
      return NextResponse.json(
        {
          message: `Failed to fetch summary: ${response.statusText}`,
    ...
    if (result?.message) {
      ...
      // summary.resumeId = body.id as string;
