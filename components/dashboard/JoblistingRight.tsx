    ...
    return (
        <div className='overflow-hidden grid gap-5 text-sm pr-5 pb-32'>
            {Array.from(arr).map((element, index) => {
                return (
                    <>
                        <Link href={`/dashboard/jobdetailse/${index}`} className=''>
                            <div key={index} className='joblisting overflow-y-auto p-5 border shadow-sm rounded-md flex items-center justify-around '>
